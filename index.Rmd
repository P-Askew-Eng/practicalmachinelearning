...
```{r}
newfit<-train(classe~accel_forearm_x+accel_forearm_y+accel_forearm_z+accel_dumbbell_x+accel_dumbbell_y+accel_dumbbell_z+accel_arm_x+accel_arm_y+accel_arm_z+accel_belt_x+accel_belt_y+accel_belt_z,data=trainset,method="rf",prox=TRUE)
newfit
plot(newfit$finalModel, uniform = T)
text(newfit$finalModel, cex=0.8)
```
